...
async def main(instances: List[str], excluded_instances: Set[str], *, ignore_prefix: bool) -> None:
    ...
    with console.status("Checking latest version..."):
        latest_version, requires_python = await fetch_latest_red_version()
    ...
    common.print_with_prefix_column(
        common.ICON_SUCCESS,
        Text("New version available: ").append(str(latest_version), style="bold"),
    )

    breaking_update = current_version.release[:2] != latest_version.release[:2]
    ...
    if breaking_update:
        parts.append(
            ...
            " or its cogs are are affected by.[/bold]\n"
        )
    parts.append(
        ...
        "Hit the [b]Enter[/] key to show to the changelog."
    )
    ...
    if current_python_version not in requires_python:
        common.print_with_prefix_column(
            common.ICON_WARN,
            "The latest version of Red requires a different version (",
            Text(str(requires_python), style="bold"),
        ...
        interpreter_exe, interpreter_version = _ask_for_interpreter(
            current_python_version=current_python_version, requires_python=requires_python
        )
    ...
    if not Confirm.ask(f"Do you want to continue with the update to [b]Red {latest_version}[/]?"):
        return
    console.print()
...
async def _check_cog_compatibility_command_impl(
    *,
    ...
    if red_version is None or python_version is None:
        with console.status("Checking latest version..."):
            red_version, requires_python = await fetch_latest_red_version()
